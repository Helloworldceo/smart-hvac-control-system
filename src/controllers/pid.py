...
class PIDController(Controller):
    def __init__(self, Kp: float = 2.0, Ki: float = 0.1, Kd: float = 5.0,
                 u_min: float = 0.0, u_max: float = 1.0, derivative_filter_tau: float = 10.0):
        self.Kp = float(Kp)
        self.Ki = float(Ki)
        self.Kd = float(Kd)
    ...
    def compute(self, measurement: float, setpoint: float, dt: float) -> float:
        ...
        derivative = alpha * raw_deriv + (1 - alpha) * self.prev_derivative
